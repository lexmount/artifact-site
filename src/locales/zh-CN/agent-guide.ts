...
export const agentGuide: Messages = {
  ...
  "A client that only starts local MCP commands can run artifact-site mcp from the CLI instead: it forwards to this server with the same token.": "只能启动本地 MCP 命令的客户端，可改用 CLI 的 artifact-site mcp：它会带同一个令牌转发到本服务器。",
  "Reload the client and check that {count} tools are available. Ask “What artifacts have I published?” without naming MCP. Then publish a test file, read it back and delete it. Use artifact_site_connection to diagnose identity or limits.": "重新加载客户端，确认有 {count} 个工具。不提 MCP，直接问「我发布过哪些作品？」。再发布测试文件、读取后删除，验收完整流程。需要检查身份或限制时，使用 artifact_site_connection。",
