...
export const agentGuide: Messages = {
  ...
  "Requires Node 24+ and npm. Run these commands from the artifact-site source repository root on the machine running your CLI or agent. The npm package is not published yet.": "需要 Node 24+ 和 npm。在运行 CLI 或 agent 的机器上，从 artifact-site 源码根目录执行以下命令。目前尚未发布 npm 包。",
  "If artifact-site is not found, check npm's global bin directory is on PATH, then restart your terminal or agent client. If npm link reports EACCES, use a user-writable npm prefix; see the CLI README for commands.": "如果找不到 artifact-site 命令，请确认 npm 全局命令目录已加入 PATH，再重启终端或 agent 客户端。若 npm link 报 EACCES，请改用当前用户可写的 npm 安装目录，具体命令见 CLI README。",
