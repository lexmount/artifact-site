    ...
    <section id="mcp" role="tabpanel" aria-labelledby="tab-mcp" hidden={mode !== "mcp"} tabIndex={0}>
      ...
      <Step number={3} title={t("Copy MCP configuration")}>
        ...
        {mcpAuth === "token" && <div>
          ...
          <details><summary>{t("Client-specific instructions")}</summary><p>{t("For Cursor, merge this entry into mcpServers in ~/.cursor/mcp.json. Other clients may use different configuration keys; choose Streamable HTTP and supply the same Authorization header.")}</p><p>{t("A client that only starts local MCP commands can run artifact-site mcp from the CLI instead: it forwards to this server with the same token.")}</p></details>
        </div>}
      </Step>
      <Step number={4} title={t("Verify the connection")}><p>{t("Reload the client and check that {count} tools are available. Ask “What artifacts have I published?” without naming MCP. Then publish a test file, read it back and delete it. Use artifact_site_connection to diagnose identity or limits.", { count: String(tools.length) })}</p><p>{t("In ChatGPT, ask “What artifacts have I published?” in a new conversation; the first answer should already list your sites. Connected applications can be reviewed and disconnected in My sites.")}</p></Step>
