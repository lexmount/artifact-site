...
export default function AppShell(props: { children: ReactNode; section?: "admin" }) {
  const inside = useContext(InsideShell);
  return inside ? props.children : <InsideShell.Provider value={true}><ShellContent {...props}/></InsideShell.Provider>;
}
function ShellContent({ children, section }: { children: ReactNode; section?: "admin" }) {
  const t = useT();
        ...
        {section === "admin" ? (
          ...
          <nav aria-label={t("Navigation")}>
            {nav.map(({ href, label, active }) => (
              <Link key={href} href={href} className={[active && "active", href === "/admin" && user && oidcEnabled && "nav-admin-desktop"].filter(Boolean).join(" ") || undefined} aria-current={active ? "page" : undefined}>{label}<NavigationStatus /></Link>
            ))}
          </nav>
        )}
