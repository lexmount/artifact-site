export function buildCommentPdf(pages: number, rotation: 0 | 90 | 180 | 270 = 0): Uint8Array<ArrayBuffer> {
  ...
  for (let i = 0; i < pages; i++) {
    objects.push(`${3 + i} 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Rotate ${rotation} /Contents ${3 + pages + i} 0 R /Resources << /Font << /F1 ${fontRef} 0 R >> >> >>\nendobj\n`);
  }
  for (let i = 0; i < pages; i++) {
