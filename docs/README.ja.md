...
> **インストールせずに試す：**[artifact-site.app.lexmount.com](https://artifact-site.app.lexmount.com/) はこのコードをホスト型サービスとして動かしています。ファイルをドロップすれば匿名で公開できます（デモ上の匿名サイトは数日で期限切れになります）。サインインすると成果物が保持されます。共有デモ環境のため、公開したものは公開情報として扱ってください。
